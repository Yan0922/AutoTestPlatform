...
def print_accuracy_summary(results: list) -> None:
    """
    按照原始 jsonl 文件路径的顺序，在控制台打印每个文件的正确率（100 - 错误率）。
    每行一条，方便复制粘贴。

    参数:
        results: list[tuple[str, float]]
            每个元素为 (jsonl_path, error_rate_percent)。
            error_rate_percent 是百分制错误率（例如 46.24 表示 46.24%）。
    """
    print()
    print("=" * 80)
    print("正确率汇总（按原始 jsonl 文件顺序输出，1 行 1 条，方便复制）")
    print("=" * 80)

    # 仅输出正确率数值（一行一个，方便整列复制到 Excel/表格）
    print("\n[ 仅正确率（每行一个值）]")
    for _, er in results:
        accuracy = round(100 - er, 2)
        print(f"{accuracy}%")

    # 输出文件名 + 正确率，方便核对
    print("\n[ 文件名 + 正确率 ]")
    for path, er in results:
        accuracy = round(100 - er, 2)
...
# if __name__ == "__main__":
    ...
    # get_wer(
    #     result_path='/home/guojun/workspace/k2_asr_model_test/asr_test_code/test_data/test_result/jianwei/ja_result.jsonl',
    #     # error_type='auto',
    #     transcription_key='en',
    # )
...
if __name__ == "__main__":
    # 指定目录路径
    directory = '/home/yanliuping/workspace/dataset/20260520_w4'

    # 收集每个 jsonl 文件的错误率，保持读取顺序
    results: list[tuple[str, float]] = []

    # 如果是目录，遍历所有 jsonl 文件
    if os.path.isdir(directory):
        # 查找目录下所有 .jsonl 文件，按文件名排序，保证顺序稳定
        ...
        results.append((directory, er))

    # 处理完所有文件后，按原始 jsonl 顺序打印正确率
